...
# Function to find AI assistant config directory
find_config_directory() {
    local standard_path="$HOME/my_new_ai_assistant"

    print_status "Looking for AI assistant configuration directory..." >&2

    if [ -d "$standard_path" ]; then
        print_success "Found AI assistant config directory at: $standard_path" >&2
        echo "$standard_path"
        return 0
    fi

    print_warning "Standard AI assistant config directory not found at: $standard_path" >&2
    echo "" >&2
    print_status "Please provide the path to your AI assistant settings directory." >&2
    print_status "This is typically located at:" >&2
    print_status "  ~/my_new_ai_assistant/" >&2
    echo "" >&2
    ...
    while true; do
        ...
        if [ -d "$expanded_path" ]; then
            print_success "Found AI assistant config directory at: $expanded_path" >&2
            echo "$expanded_path"
            return 0
        else
            print_error "Directory does not exist: $expanded_path" >&2
            echo "" >&2
